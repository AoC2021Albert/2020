m = [["eyr:2027 hcl:#602927 hgt:186cm byr:1939 iyr:2019 pid:552194973 ecl:hzl"],
...
def validyear(v, ini, end):
    v = int(v)
    return (v>=ini and v <= end)
...
def validhgt(h):
    ...
    try:
        ...
        return(False)
    if measure == 'in':
        return (validyear(i, 59, 76))
    elif measure == "cm":
        return (validyear(i, 150, 193))
    return(False)
...
count = 0
required= {
"byr": lambda x: validyear(x, 1920,2002),
"iyr": lambda x: validyear(x, 2010,2020),
"eyr": lambda x: validyear(x, 2020,2030),
"hgt": validhgt,
"hcl": validhex,
"ecl": validecl,
"pid": validpid,
...
for s in m:
    ...
    for f in s.split(' '):
        k,v = f.split(':')
        pks.add(k)
        valid = valid and required[k](v)
    if valid and requiredset.issubset(pks):
        count += 1
print(count)
