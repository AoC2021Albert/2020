...
print(sum(len(st) for st in (set().union(*g) for g in m)))
print(sum(len(st) for st in (set(g[0]).intersection(*g) for g in m)))

ret = 0
for g in m:
    st = set(list(g[0]))
    for s in g:
        st.intersection_update(list(s))
    ret += len(st)
print(ret)

#print(sum(len(s) for s in [] ))