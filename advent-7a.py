m= '''posh brown bags contain 5 dim coral bags, 1 plaid blue bag, 2 faded bronze bags, 2 light black bags.
...
insidek=defaultdict(list)
outside=defaultdict(list)
for l in m.split('.\n'):
    parent, children = l.split(' bags contain ')
    children=children.replace(' bags.','')
    children=children.replace(' bag.','')
    children=children.replace(' bags','')
    children=children.replace(' bag','')
    for child in children.split(', '):
        number=child.split(' ')[0]
        if number != 'no':
            bagkind = child[len(number)+1:]
            number = int(number)
            inside[parent].append((bagkind,number))
            insidek[parent].append(bagkind)
            outside[bagkind].append((parent))#,number))
ret = set()
new_set = set(outside[endbag])
...
while new_set:
    ...
    current_set=new_set
    new_set=set()
    for bagkind in current_set:
        new_set.update(outside[bagkind])
    new_set = new_set - ret
print(ret)
print(len(ret))

print(inside)
def countinnerbags(kind):
    print(f"checking {kind}")
    return(sum(number * (1 + countinnerbags(bag)) for bag, number in inside[kind]))

print(countinnerbags(endbag))
print(inside[endbag])